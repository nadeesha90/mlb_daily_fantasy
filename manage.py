#!/usr/bin/env python
"""Main entry-point into the 'dfs' Flask and Celery application.
...
from dfs_portal.application import create_app, get_config
...
def setup_logging(name=None):
    ...
    if log_to_disk:
        file_name = os.path.join(OPTIONS['--log_dir'], 'dfs_portal_{}.log'.format(name))
...
def parse_options():
    ...
    if OPTIONS['--config_prod']:
        config_class_string = 'dfs_portal.config.Production'
    else:
        config_class_string = 'dfs_portal.config.Config'
...
def celeryworker():
    ...
    app = create_app(parse_options(), no_sql=True)
    Logging._setup = True
    celery_args = ['celery', 'worker', '-n', OPTIONS['--name'], '-C', '--without-gossip']
