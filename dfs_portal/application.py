...
import dfs_portal as app_root
from dfs_portal.blueprints import all_blueprints
from dfs_portal.extensions import celery, db, mail, redis
...
def get_config(config_class_string, yaml_files=None):
    ...
    # Expand some options.
    celery_fmt = 'dfs_portal.tasks.{}'
    db_fmt = 'dfs_portal.models.{}'
    ...
    yaml_files = yaml_files or [f for f in [
        os.path.join('/', 'etc', 'dfs_portal', 'config.yml'),
...
def create_app(config_obj, no_sql=False):
    ...
    with app.app_context():
        import_module('dfs_portal.middleware')
