...
def get_attr(sqla_object, attribute):
    ...
    output = sqla_object
    def deep_getattr (elem, attr):
        fullDepth = attr.split('.')
        output = getattr(elem,fullDepth[1])
        for child in fullDepth[2:]:
            output = getattr(output, child)

        return output

    for x in attribute.split('.')[:2]:
        if type(output) is InstrumentedList:
            output = ','.join([str(deep_getattr(elem, attribute)) for elem in output])
        else:
...
class DataTables:
    ...
    def sorting(self):
        ...
        for sort in sorting:
            ...
            ordering = obj.asc() if sort.dir == 'asc' else obj.desc()
